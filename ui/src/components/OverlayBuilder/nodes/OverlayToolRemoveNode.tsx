...
import type { OverlayToolRemoveNodeType } from "../types";
import { useTools } from "@/api/apiCalls";
import { useNamespace } from "../NamespaceContext";

export function OverlayToolRemoveNode({ data, id, selected }: NodeProps<OverlayToolRemoveNodeType>) {
  ...
  return (
    <div className={`canvas-node context-skills-node${data.disabled ? " canvas-node--disabled" : ""}`}>
      ...
      <div className="canvas-node__header context-skills-node__header">
        <span className="canvas-node__icon">🗑️</span>
        <span className="canvas-node__title">Tools (Remove)</span>
        <button
      ...
      <Handle type="source" position={Position.Right} id={`toolrm-out-${id}`} className="handle--context-skills" />
    </div>
  );
}
