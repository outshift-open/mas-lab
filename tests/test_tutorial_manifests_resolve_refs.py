...
"""Resolve-refs validation gate for mas-lab tutorial runtime manifests."""
...
def _tutorial_ref_paths() -> list[tuple[Path, str]]:
    root = mas_lab_root()
    if not root.is_dir():
...
def test_tutorial_runtime_manifest_resolve_refs(path: Path, kind: str):
    """Tutorial agent/overlay/mas/deployment YAML refs resolve on disk."""
    pytest.importorskip("jsonschema")
    result = validate_file(path, kind=kind, strict=True, resolve_refs=True)
