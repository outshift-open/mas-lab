...
[contributing](CONTRIBUTING.md) to `MAS-lab`.
...
MAS Lab configuration uses environment variable **names** (for example
`api_key_env: OPENAI_API_KEY`), not inline secret values. Never commit API
keys, tokens, or private keys to the repository.

## Dependency updates

Keep workspace dependencies current via `pyproject.toml` / `uv sync`. Report supply-chain concerns
