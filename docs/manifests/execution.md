...
Engine queue depth, cache policy, streaming, and parallel tool dispatch
...
| --- | --- |
| `spec.execution` (cache, stream, queue depth, …) | `kind: RuntimeEngine` via `runtime_refs` / `--runtime-ref` / `MAS_RUNTIME_REFS` |
| Offline LLM turns | [llm_cache replay](llm-cache.md) recorded against a live provider (`raise_on_miss`) |
